...
import { useUnreadCount } from '@/hooks/useUnreadCount'; // Importando o hook que criamos
...
const Sidebar: React.FC<SidebarProps> = ({ user, activeTab, setActiveTab, onLogout }) => {
  // Hook que monitora o Supabase Realtime para mensagens não lidas
  ...
  return (
    <div className="w-64 bg-slate-900 h-screen flex flex-col flex-shrink-0 text-slate-300">
      <div className="p-6 border-b border-slate-800 flex items-center gap-3">
        <div className="w-8 h-8 bg-indigo-500 rounded-lg flex items-center justify-center font-bold text-white shadow-lg shadow-indigo-500/20">Z</div>
      ...
      <nav className="flex-1 py-6 px-4 space-y-1 overflow-y-auto custom-scrollbar">
        {filteredMenuItems.map((item) => (
          ...
          >
            ...
            {/* Exibição do Badge de Mensagens Não Lidas */}
            {item.id === 'conversations' && unreadCount > 0 && (
              <span className={`flex h-5 min-w-[1.25rem] px-1 items-center justify-center rounded-full text-[10px] font-black transition-all ${
      ...
      <div className="p-4 border-t border-slate-800">
        <button 
          onClick={onLogout}
          className="w-full flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-rose-500/10 hover:text-rose-400 text-slate-400 transition-colors group"
        >
