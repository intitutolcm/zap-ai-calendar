...
const CalendarPage: React.FC<CalendarPageProps> = ({ showToast }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Se salvou/editou como CONFIRMADO, dispara
      if (formData.status === 'CONFIRMED' && resultId) {
        await triggerNotification(resultId);
      }
    ...
    <div className="p-8 h-full overflow-y-auto custom-scrollbar bg-slate-50/30">
      ...
      {activeTab === 'calendar' ? (
        ...
        <div className="bg-white border border-slate-200 rounded-[2.5rem] overflow-hidden shadow-sm">
          {/* Contêiner de Scroll Lateral */}
          <div className="overflow-x-auto custom-scrollbar">
            <table className="w-full text-left min-w-[800px]"> {/* min-w garante que o scroll apareça */}
              <thead>
              ...
              <tbody>
                {filteredAppointments.length > 0 ? (
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          {ap.status === 'PENDING' && (
                            <button 
                              ...
                              className="p-2 text-indigo-600 hover:bg-indigo-50 rounded-xl transition-all shadow-sm border border-indigo-100 shrink-0"
                          ...
                          {/* Botão de Edição Rápida na Lista */}
                  ...
                  <tr>
                    <td colSpan={4} className="px-8 py-10 text-center text-slate-400 font-medium">Nenhum agendamento encontrado para este período.</td>
                  </tr>
                )}
              </tbody>
            </table>
                    ...
                    <div key={ap.id} className="flex justify-between items-center p-4 bg-slate-50 rounded-2xl border border-slate-100">
                      ...
                      <div className="flex items-center gap-3">
                        <select 
                          value={ap.status} 
                          onChange={(e) => updateAptStatus(ap.id, e.target.value)}
                          className={`text-[9px] font-black uppercase px-2 py-1 rounded-lg border-none ${STATUS_CONFIG[ap.status]?.bg} ${STATUS_CONFIG[ap.status]?.color}`}
                        >
            ...
            {formData.date < todayStr ? (
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <select required className="px-5 py-4 rounded-2xl bg-slate-50 border-none" value={formData.serviceId} onChange={e => setFormData({...formData, serviceId: e.target.value})}>
                    <option value="">Serviço</option>
                    {formDeps.services.map((s: any) => <option key={s.id} value={s.id}>{s.name}</option>)}
                  </select>
                  <select required className="px-5 py-4 rounded-2xl bg-slate-50 border-none" value={formData.professionalId} onChange={e => setFormData({...formData, professionalId: e.target.value})}>
                    <option value="">Profissional</option>
                    {formDeps.professionals.map((p: any) => <option key={p.id} value={p.id}>{p.name}</option>)}
                  </select>
                </div>
                <div className="grid grid-cols-3 gap-4">
                  <input type="date" className="px-5 py-4 rounded-2xl bg-slate-50 opacity-60" value={formData.date} readOnly />
                  <input type="time" required className="px-5 py-4 rounded-2xl bg-slate-50" value={formData.time} onChange={e => setFormData({...formData, time: e.target.value})} />
                ...
                <div className="flex gap-4 pt-4">
                  ...
                  <button type="submit" className="flex-2 py-4 bg-indigo-600 text-white rounded-2xl font-bold shadow-lg">{editingId ? 'Salvar Alterações' : 'Confirmar Agendamento'}</button>
                </div>
              </form>
            )}
          </div>
